...
**Help pages:** `man/` is generated by roxygen2 and is not committed, and
`install_github()` installs the repository as-is without regenerating it — so an
install straight from GitHub works, but ships without help pages. To get them,
clone the repository, regenerate the documentation, and install locally:

```r
devtools::document("path/to/raffalib-r")  # regenerates man/
devtools::install("path/to/raffalib-r")
```

For development use, the intended workflow is `devtools::document()` followed by
`devtools::load_all()` from the cloned checkout; `document()` regenerates
`man/` on the spot, so the help pages are always current.
...
| `gtsummary_add_mean_diff()`, `gtsummary_add_significance_stars()`, `gtsummary_collapse_footnote_newlines()`, `gtsummary_format_statistic_column()`, `gtsummary_rename_column()` | Extensions for `gtsummary` tables: between-group differences — `{mean}`/`{median}` for continuous variables, `{col_pct}`/`{row_pct}`/`{cell_pct}` percentage-point differences for categorical and dichotomous ones, combined per variable through a `tbl_summary`-style `statistic` formula-list-selector of glue templates (e.g. `"{mean}\n{median}"`), with per-variable rounding overrides through a matching `digits` selector and an auto-built footnote naming the groups, the direction and the selected statistics — custom significance stars, collapsing the footnote line breaks that multiline statistic templates leak, statistic-column formatting, and column renaming that keeps the styling references intact. |
...
`save_backup()` / `read_backup()` — time-stamped snapshots of any object
(`<stem>_YYYY-MM-DD_HH-MM-SS.qs2`/`.rds`). The `format` argument picks the
serialisation: the multithreaded `qs2` format (default), zstd-compressed rds,
gzip rds, or uncompressed rds. `save_backup()` refuses to write a
